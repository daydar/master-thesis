...
So gibt es den Ansatz des regelbasierten Systems bzw.\ des Produktionssystems, bei dem die Schlussfolgerung über Regeln und Fakten, 
die von einer/em Benutzer:in definiert werden, konstruiert wird.
...
des von der Inference Corporation entwickelten Expertensystems \textit{ART} für die Regeln und Fakten abgeleitet wurde und damit dieser sehr ähnelt~\cite{haleyaiHaleyARTSyntax2008}. 
...
welche sich bis auf die Datenbankintegration nur marginal in der Nutzung der Regel und Faktendefinitionen unterscheidet~\cite{https://doi.org/10.1111/j.1468-0394.1990.tb00164.x}
und für dies keine aktive Entwicklung und Wartung existiert. 
Andere moderne Umsetzungen konzentrieren sich im Vergleich eher auf die Inferenzmaschine, die auch als Regelinterpreter gilt.
...
eignen sich sowohl Nutzungsschnittstellen bzw. User Interface (UI) Bibliotheken für den Desktop als auch für den Webbereich.
Die Priorität liegt hierbei bei der Erstellung der einzelnen Komponenten der graphische Benutzungsoberfläche bzw.\ der Graphical User Interface (GUI). 
Folglich können Projekte wie \textit{Qt}~\cite{qtQtCrossplatformSoftware} und \textit{ImGui}~\cite{omarDearImGui2022}, 
welches in C++ geschrieben ist, Bibliotheken bereitstellen, 
die auf einer nativen Ebene funktionieren und damit in verschiedenen Anwendungsfällen genutzt werden.
Die letztere genannte Bibliothek benutzt intern einen Zustandsautomat, um die Render-Pipeline darzustellen, 
was etwas mehr Erfahrung mit der Grafikprogrammierung erfordert, dass jenes verwendet werden kann~\cite{furhtCreatingUserInterface2018}.

Dem gegenüber stehen abstraktere UI-Lösungen, welche im Front-End Bereich verwendet werden, wie das Benutzen eines Web Frameworks.
...
Weitere Kandidaten wie \textit{Svelte}~\cite{svelteSvelteCyberneticallyEnhanced} versuchen durch das Nutzen eines Complier, 
...
Weitere neue Projekte wie \textit{Fonduer}~\cite{wu2018fonduer} fokussieren sich mit Hilfe von schwachem überwachten Lernen (supervised learning) 
...
Für das Teilsystem der symbolischen künstlichen Intelligenz habe ich den Ansatz bzw.\ 
...
zusammen und bietet die Möglichkeit durch weitere Klassendefinitionen, die den Mechanismus der Vererbung nutzen, an, Abbildungen von unterschiedlichen Dokumententypen zu kreieren.
...
dass diese nicht auf eine Seite passt, weswegen die Seitennummern relevant für die Beschreibung des Zielbereiches ist.
Diese Seitennummern können den Funktionen neben den Bounding Boxen mitgegeben werden.
Daraus folgend werden diese zwei Eigenschaften als notwendige Bedingung für jegliche Extraktionsbereiche behandelt und 
...
Die Methoden nutzen dann die Angabe der Seitennummer und der Bounding Box um zunächst den Bereich der Extraktion einzuschränken.
Mit dem ausgewählten Bereich werden dann die Funktionalitäten für die Text- oder Tabellenextraktion aufgerufen.

Bei den prototypischen Dokumenten fällt zunächst wieder die hinderliche Tabellenextraktion von unkonventionellen Tabellen 
und indirekt von benutzerdefinierten Tabellen auf.
So können die Bibliotheken diese Tabellen mit den dafür konzipierten Funktionen nicht erkennen 
bzw.\ von diesen keine strukturellen Daten, die die Tabellen abbilden sollen, erhalten.
...
So werden weitere Eigenschaften wie die Angabe eines Start- und Endankers benötigt, um so nur die Zeilen aus dem Rohtext zu erhalten.
Da bei den elektronisch verarbeiteten Dokumente die Tabellen als Texte mit Zeilenumbrüchen verfasst sind, 
können die Zeilenumbrüche genutzt werden um so aus den Zeilen eine Liste zu generieren.
Die Liste wiederum wird mit regulären Ausdrücken weiter verarbeitet, womit die Daten durch Zuweisungen eine strukturierte Form erhalten 
und somit die Semantik der Tabellen darstellen.
...
die Start- und Endanker als Voraussetzung für jene Extraktionsfunktionalitäten. 
Optional bleiben die Angabe der Zwischenanker.

Die Klassen der einzelnen Dokumentarten dienen für weitere Funktionalitäten, die spezifischere Zwecke 
...
In Kapitel~\ref{sec:auf} wurde die Struktur des gesamten Systems definiert.
Des Weiteren wurde für das System die Client-Server Architektur festgelegt. 
...
Über die Umgebungsvariablen erhält dann die Flask Instanz ihre Daten bezüglich der Konfiguration für das Hosting.
...
Bei dieser Idee wird eine Factory Funktion geschrieben, die neben der Kreierung einer Instanz, 
...
und dann werden die Route Funktionalitäten deklariert~\cite{flaskAPIFlaskDocumentation}.
...
Damit die Steuerung bzw.\ die Anfrage für die Datenextraktion den Benutzer:innen zugänglich gemacht wird, 
...
So wird eine Obermenge definiert, die als Hierarchie eine Teilmenge von Textfeldern, eine Teilmenge von konventionellen Tabellen, 
...
welche abhängig von der Regelaktion angegeben werden. 
...
Regelbasierte Systeme wie \textit{CLIPS} nutzen, wie bereits beschrieben, eine angepasste Version der Syntax von der Programmiersprache Lisp.
Diese Syntax bildet mit Hilfe von Schlüsselwörtern die Ausdrücke für Regeln und Fakten ab.
Python ermöglicht Texte mittels der integrierten \(eval()\) Funktion zu evaluieren und falls es sich um eine Python Aussage handelt, 
diese auch auszuführen.
Die Funktion \( literal\_eval()\) aus dem Python-Modul \textit{Abstract Syntax Tree} (ast) erlaubt 
die selbe Funktionalität mit einer eingeschränkten Untermenge für die Python Ausdrücke.
Hierbei werden bei beiden Funktionalitäten die Texteingaben mithilfe des Parsings in abstrakte Syntaxbäume abgebildet, 
...
Dafür wird eine Grammatik benutzt, die die Ausdrücke mit Hilfe von Regeln in ihre Typen und Namen als Bestandteile auflöst.
...
Hierfür wird die Parsing Bibliothek Lark benutzt, welches die Erstellung einer kontextfreien Grammatik
in der Erweiterten Backus-Naur-Form (EBNF) erlaubt.
...
Dieser wird dann dem Parser mitgegeben, womit jeglicher Regel- und Faktenausdruck durch einen abstrakten Syntaxbaum 
der Logik für die Evaluation dieser Regel bereitgestellt wird.
Die booleschen Ausdrücken werden in der Syntax von Python genutzt, womit die Evaluation direkt stattfinden kann.

Der Aufbau der umgesetzten  Grammatik basiert hierbei auf der klassischen Aussagenlogik und ihrer Definitionen\cite{klement2004propositional}.
...
Diese stellt eine aussagenlogische Oder-Verknüpfung beziehungsweise eine Disjunktion dar, 
...
Für die \(operator\) Regel bestehen die Mehrdeutigkeiten aus der \(comp\_operator\) Regel, der \(ident\_operator\) Regel und der \(memb\_operator\) Regel, 
welche in der Reihenfolge aufgezählt die Vergleichsoperatoren, die Identitätsoperatoren und die Memberoperatoren abbilden.
Arithmetische Operatoren, Zuweisungsoperatoren und bitweise Operatoren, die in Python existieren, 
werden für die aus der Grammatik erzeugten Sprache nicht definiert. 
...
Abbildung~\ref{fig:syntaxbaum} zeigt ein Beispiel wie somit aus einem Regelausdruck ein Syntaxbaum durch den Parser kreiert werden kann.
Mittels der Transformer Klasse der Lark-Bibliothek wird nämlich als Zwischenschritt 
die Syntax der einzelnen Blätter und Knoten des Baums so angepasst, 
dass diese für die weitere Verarbeitung beziehungsweise Evaluation der Regelmaschine bereitstehen.


\subsection{Regelmaschine}\label{sec:ruleeng}

Nachdem nun die Syntax und Grammatik der Regeln gegeben ist, wird die Regelmaschine als Modul entwickelt.
...
Jedoch erlaubt der Ansatz der Speicherung der Regeln und Fakten über eine Datei im Dateisystem den schnellen Zugriff 
und die schnelle Bearbeitung jener.
Des Weiteren kommt hier die Frage auf, in welchem Dateiformat die Regeln und Fakten festgehalten werden.
...
\begin{figure}[htb]
    ...
    \caption{Durchlauf der Regelinterpretationen durch Request}
...
müssen aus den Fakten nach dem Laden während der Laufzeit eine für die Evaluation angepasste Version generiert werden.
...
Dies Konvertierung der Fakten ist für die Auswertung mit den Regeln im späteren Prozess geeignet.
...
Die Funktionalitäten des Interpreters werden dann aufgerufen, 
sobald die HTTP Anfrage bezüglich des Durchlaufens der Regelevaluation im WSGI ankommt.
Die Abbildung~\ref{fig:regelmaschine_sequenz} zeigt den Verlauf bei dieser Anfrage.
So löst zunächst die Route-Anfrage die gekoppelte Funktion \(trigger_run()\) der Flask Instanz aus, 
welche wiederum die \(run()\) Methode der Regelmaschine auslöst. 
Diese ruft dann die \(run()\) Methode des Regelinterpreters mit der Angabe für welchen Dokumentart der Vorgang stattfinden soll.
Dort werden iterativ die Regeln mit Hilfe der Funktion     
