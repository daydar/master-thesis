...
PDF-Dokumente besitzen viele Informationen, aus denen sich neue Daten generieren lassen.
Doch das Extrahieren von solchen Daten ist heutzutage immer noch mit Hürden verbunden.
Dies gilt auch für die Dokumente, die für die einzelnen Arbeitsprozesse von Autohäusern und deren Kfz-Werkstätten verwendet 
...
Der erste Ansatz handelt um die Nutzung der symbolischen künstlichen Intelligenz, 
während demgegenüber der zweite Ansatz die modernen Techniken des maschinellen Lernens für das Lösen dieser Problematik nutzt.
...
Gemeint war damit die Forderung nach einer zunehmenden Digitalisierung im privaten Alltag vieler Bürger:innen, 
...
Eine Extraktion der Daten der einzelnen Dokumente ist jedoch nur eingeschränkt möglich, 
...
was bei einer enormen Menge an Dokumente zu einer monotonen Arbeit für die Händler:innen der Autohäuser führt. 
...
Die Technologien der Datenextraktion benutzen unterschiedliche Ansätze und müssen daher zunächst für diesen Fall ausgewählt werden. 

Damit jedoch auch für Agierende innerhalb des technischen Bereichs die Nutzung nicht zu abstrakt bleibt, 
soll durch die Entwicklung meines Systems der Zugriff greifbarer gestaltet werden, 
welches wiederum allein durch eine benutzungsfreundliche Anwendung ermöglicht wird. 
...
die hingegen bei einer Erfüllung weitere Aktionen oder Regeln auslösen können.
...
Hiermit bezeichnet man einen unzeitgemäßen Ansatz der KI, bei der über das Festlegen von Symbolen 
...
Das heißt, die Benutzer:innen eines solchen Systems sollen sich beginnend von groben Definitionen für die Verarbeitung 
...
Hierbei werde ich im Laufe der Entwicklung feststellen, welcher Typ des Machine Learnings sich für diesen Fall am optimalsten eignet.
...
Des Weiteren soll es das System ermöglichen, Änderungen der Definitionen anzumerken und Unterschiede zu erstellen. 
...
für die Entwicklung eingerichtet werden, mit der die Implementierung der Systeme stattfindet. 
...
die Eigenschaften einer solchen KI durchgeführt werden, 
...
zwischen zwei verschiedenen Ansätze aus der Künstlichen Intelligenz.

Im Anschluss daran wird das Software Testing für die beiden Subsysteme eingeführt. 
...
sich hiernach mit den Systemen verbinden soll, um infolgedessen die Ausführbarkeit der Funktionalität überprüfen zu können. 
...
Dazu stehen als weitere Grundlage mehr als 10.000 annotierte unterschiedliche Dokumente 
mit genau den zu extrahierenden Daten als Datenbank zur Verfügung. 
...
mit der Autohäuser ihre Arbeitsprozesse in Schrift, Sprache und Bild dokumentieren.
Die Dokumentation erfolgt mittels mobiler Endgeräte und wird an das ERP-System weitergeleitet, 
...
Sie können extrahiert werden, sobald Änderungen der Daten beobachtet oder mitgeteilt worden sind. 
...
Zum Beispiel nutzen Produkte wie Textractor~\cite{amazonIntelligenteExtraktionText} von AWS oder 
...
Das in Abbildung~\ref{fig:vd} zu sehende Debugging ermöglicht, ermöglicht es unter anderem, 
...
oder Verarbeitung versehentlich dieselben Einträge erneut im Quellcode des PDFs besitzen. 
...
Für Tabula gilt hier hingegen die Ausnahme, dass für das Projekt unterschiedliche Wrapper für weitere Programmiersprachen wie Python existieren.
Hieraus ist zu beobachten, dass die Nutzung von Python viele mögliche Extraktionswerkzeuge erlaubt, 
die sich für die spätere Umsetzung als noch bedeutsam erweisen.
...
Zudem fehlen jegliche Einrahmungen oder jegliche Trennlinien für Zeilen und Spalten.
...
Die Tabelle ist an der Kopfzeile zu erkennen, die die Jobs und Joblines in die Arbeitsnummer/Teilenummer, 
in die Bezeichnung, in die Menge des Arbeitszeitwertes, in den Preis und in den Betrag übersichtlich aufteilt.
Die Teilenummern beziehen sich hierbei auf die Identifikationsnummer der Reparaturteile.
...
in mehreren Zeileneinträgen als Überläufe bei den Arbeitsnummern beziehungsweise den Teilenummern zu sehen.
...
Dies ist durch die Codierung der Arbeitsnummern beziehungsweise der Teilenummern zu erkennen, 
...
So sind die Codes der Jobs als eine fünfstellige Zeichenkette aus Ziffern bestimmt, die mit den Ziffern \textit{00} anfangen. 
...
In Abbildung~\ref{fig:formel1_r} wird eine Rechnung der Formel 1 DMS gezeigt.
...
Auch dieses hier abgebildete Dokument wurde anonymisiert. 
...
zwischen den Zeilen ein Art Summenstrich hinzugefügt wird, der die Anzahl der Jobs anzeigt.
...
durch eine gemeinsame Problematik miteinander verbunden sind.
...
Diese Tabellenform besitzt außerdem selten Trennlinien, welche essentiell sind, um eine derartige Tabelle als solche definieren zu können.
...
jedoch bricht sie in den prototypischen Dokumenten aber mindestens 
...
Dies tritt für Dokumente auf, welche eine längere Auflistung der Jobs und Joblines besitzen 
und somit auf weiteren Seiten dargestellt werden müssen.
...
Es ist festzustellen, dass verschiedene Tabellenstrukturen für unterschiedliche Dokumentarten existieren.
...
sodass dadurch eine Verallgemeinerung verschiedener Dokumententypen ermöglicht wird.
...
und Fakten abgeleitet wurde und damit zahlreiche Gemeinsamkeiten und Ähnlichkeiten aufweisen.~\cite{haleyaiHaleyARTSyntax2008}. 
...
und für das keine aktive Entwicklung und Wartung existiert. 
...
Folglich können Projekte wie \textit{Qt}~\cite{qtQtCrossplatformSoftware} und das in C++ geschriebene \textit{ImGui}~\cite{omarDearImGui2022} Bibliotheken bereitstellen, 
...
sind hier gleicherweise ML-Frameworks eine geeignete Lösung.
...
Weitere neue Projekte wie \textit{Fonduer}~\cite{wu2018fonduer} fokussieren sich mit Hilfe 
von schwachem überwachten Lernen (weak supervised learning) 
...
weswegen sich weak supervised learning für einfache Dokumente als praktisch erweisen kann.
...
sobald die benutzerdefinierten Regeln mit den bereits vorhandenen Fakten erfüllt worden sind, als Funktionalitäten.
...
sodass der Bereich der Extraktion zunächst limitiert werden kann.
...
und dadurch von diesen keine strukturellen Daten, die die Tabellen abbilden sollen, erhalten.
Um dies zu umgehen, werden als Lösungswege die unkonventionellen und die benutzerdefinierten Tabellen 
zunächst als reine Texte extrahiert und danach die Einträge der jeweiligen Felder mit Hilfe von regulären Ausdrücken gefiltert.
...
Da bei den elektronisch verarbeiteten Dokumenten die Tabellen als Texte mit Zeilenumbrüchen verfasst sind, 
...
dieselbe Funktionalität mit einer eingeschränkten Untermenge für die Python Ausdrücke.
...
braucht es eine für den Anwendungsfall angepasste Definition einer Grammatik.
...
mit diesem die Regelmaschine nur eine Regelspeicher Instanz referenzieren kann.
...
So können beim initialen Aufruf die letzten Werte geladen werden, sodass diese Anwendung offline-fähig dargestellt werden kann.
...
Auf der Benutzungsoberfläche wird dann der Texteditor angezeigt, der auch die Navigation über Tabs unterstützt.
...
Dies wurde dynamisch mit der Hover-Funktion umgesetzt, mit dieser die Benutzer:innen direkt die X- und Y-Koordinaten erhalten können.
...
Beim zweiten Fenster werden Informationen, die abseits von Ergebnissen, wie Zustände des Systems oder Fehlermeldungen angezeigt.
Dieses Fenster soll es ermöglichen, einen aktuellen Zustand und eine Rückmeldung zu präsentieren, 
sodass im Zuge dessen die Mensch-Computer-Interaktion verbessert und die Nutzungsbarriere zum System niedriger wird.
...
Neben dem maschinellen Lernen existieren unter anderem die Arten des überwachten Lernens (supervised Learning) 
und des nicht überwachten Lernens (unsupervised Learning).
Beide haben ihre Einsatzgebiete auch innerhalb der Dokumentenextraktion.
Hierbei zählen zu dem Bereich der Extraktion verschiedene Dokumentarten wie Text- oder HTML Dokumenten hinzu, 
bei welchem die letztere genannte Art 
präzise Ergebnisse bei der Verwendung der Methoden des überwachten Lernens verschafft~\cite{gopinath2018automatic}.
Nicht überwachtes Lernen hat im Gegensatz zu Dokumenten, die kein Labeling besitzen, einen höheren Wirksamkeit.
...
Das semi-überwachte Lernen verhilft das Labeling durch weitere Annahmen der Struktur 
unter Zuhilfenahme von semi-überwachter Klassifikation und durch eingeschränktes Clustering zum Durchbruch~\cite{zhu2009introduction}.
Das schwache überwachte Lernen verwendet als Technik die Datenmodelle über neu generierte Daten.
Dies ist für den Fall vorgesehen, wenn man mit neuen Daten aus Dokumenten arbeiten möchte.
In solch einem Fall eröffnet die Bibliothek Fonduer mit der Erstellung einer Pipeline-Struktur eine neue Lösungsmöglichkeit.
...
Genauer gesagt wird hier die Idee des Knowledge Base Construction (KBC), 
...
Daher besteht für die Nutzung dieser Bibliothek eine Abhängigkeit von dem Datenbanksystem PostgreSQL~\cite{elmasriFundamentalsDatabaseSystems2007}. 
Des Weiteren wird die Bibliothek spaCy genutzt, weswegen die Sprachmodelle für die deutsche Sprache 
in dem Dockerfile des Teilsystems mitgeladen und verlinkt werden.
Über die Kompositionsdatei von Docker werden dann die Container für die Machine-Learning Komponente 
und die dazugehörige Datenbank virtuell aufgesetzt und mit dieser entwickelt.

Damit dies auch für vielschichtige Daten funktioniert, wird hier ein spezielles Datenmodell benutzt, 
...
Diese Eigenschaft wird als Multimodalität bezeichnet und existiert in der Prozesskette an verschiedenen Stellen.
...
Die Menge der Dokumente für die jeweilige Art unterscheiden sich in ihrer Dimension:
...
So sind die einzelnen Phasen — aufgeteilt in zwei Kategorien — zu sehen. 
...
In der ersten Phase wird zunächst die KBC über ein spezielles Schema
und die daraus folgenden iterative Erstellung eines Eingabekorpus initialisiert.
Dafür wird die Hostadresse der benötigten Datenbank angegeben und dergleichen einem Meta-Objekt, welches die Brücke zur Persistenz darstellt, übergeben, 
...
welche den Pfad der zu übersetzenden Dateien bei Instanziierung mitgegeben wird.
...
Ein weiterer Weg wäre es, dies über den HTMLPreprocessor umzusetzen, was aber bei digital verarbeiteten Dateien nicht von der 
...
und über weitere Parametrisierung die Inklusion der strukturellen und sprachlichen Informationen angibt.
In der zweiten Phase werden Mentions, welche die potentiellen Kandidaten für die Datenextraktion darstellen, definiert.
In dem Anwendungsfall des Projekts wird für jedes Feld, das sich in den Aufträgen und den Rechnungen befindet 
und extrahiert werden soll, eine Mentions festgelegt.
Beispiele hierfür sind, wie aus dem Teilsystem der symbolischen KI bereits bekannt, die Jobs, die Reparaturzeiten, 
...
die über eine Filterangabe eine präzise Erkennung geben sollen.
Die Matchers gibt es in verschiedenen Variationen. 
In diesem Anwendungsfall wird sich für die Filterung über reguläre Ausdrücke entschieden, 
da diese bei der symbolischen KI ihre Anwendungen finden.
...
Nachdem diese Informationen auf das MentionExtractor-Objekt angewendet werden, 
wird das Schema der Kandidaten für die Extraktion angegeben.
Hierfür braucht es eine Definition für jede Dokumentenart und ihre eigene Unterklasse der Candidate-Klassen.
Folglich wird ein Warranty-Objekt für die Care-Aufträge, ein Invoice-Objekt für die Formel 1 Rechnungen 
und ein Invoice-Objekt für die KFZ3000 Rechnungen definiert.
Diesen wird wie bei der Mentions-Klasse dieselben Matcher überreicht.

Dazu werden Throttlers, das sind Funktionen, die als weitere starke Filter fungieren, 
definiert, um die Genauigkeit der validen Extraktionskandidaten zu ermöglichen. 
In diesem Anwendungsfall definieren wir daher Throttler-Objekte, 
die sich mit den Ausrichtungen der verschiedenen Einträgen beschäftigen.
...
Hierfür wird ein Featurizer-Objekt definiert, welchem die Verbindung zur Datenbank 
und den Kandidaten der jeweiligen Dokumentenart übergibt wird.
In der vierten Phase findet das schwache überwachte Lernen und die Klassifikation über Labeling-Funktionen statt.
Dafür werden diese Funktionen, welche weitere Bedingungen für die Kandidaten stellen und 
die es zu erfüllen gilt, definiert, um als valider Kandidat weiterverarbeitet zu werden.
...
In dem Listing~\ref{code:labeling} werden zwei Beispiele der für das Teilsystem implementierte Label-Funktionen dargestellt.
Hierfür wird neben den zwei gängigen Wahrheitswerten ein Wahrheitswert mit dem Schlüsselwort ABSTAIN definiert, 
welches gesetzt wird, wenn keine Entscheidung für eines der zwei Wahrheitswerte TRUE oder FALSE 
zu dem ausgeführten Zeitpunkt für das Labeling möglich ist.
...
\begin{listing}[htbp]
    ...
    \begin{lstlisting}
        ...
        @labeling_function()
        def LF_repair_times_row(c):
            if 'repair_times' in get_row_ngrams(c.description):
                return TRUE  
            else:
                ABSTAIN
        
        @labeling_function()
        def LF_stamp_time_row(c):
            if 'stamp_time' in get_row_ngrams(c.description):
                return TRUE  
            else:
                ABSTAIN
        
    \end{lstlisting}
    \caption{Labeling-Funktionen für die Supervision}
    \label{code:labeling}
\end{listing}
...
Dafür benutzt Fonduer das Machine-Learning Framework Emmental, das die Funktionalitäten für die Modellierung bereitstellt~\cite{wu2019emmental}.
...
Dass diese nötigen Eigenschaften für das Teilsystem erfüllt werden können, 
wird derselbe Ansatz des als erstes vorgestelltes Teilsystems umgesetzt.
Das bedeutet eine Nutzung der WSGI Schnittstelle,  
die auch die Verwendung einer Flask Anwendung als Webserver zur Folge hat. 

So wird auch eine Factory-Methode für verschiedene Anwendungsfälle, 
wie unter anderem die Continuous Integration, bereitgestellt.
...
Nachdem ich nun im bisherigen Verlauf meine Vorgehensweise und Durchführung geschildert habe, zeige ich nun meine Ergebnisse:

Mein Projekt hat insgesamt verschiedene Merkmale meiner eingangs geschilderten Problematik hervorgehoben.
So wurde im Zuge meiner Analyse zuerst gezeigt, dass eine weitere Verarbeitung der aus den Autohäusern entstandenen Dokumente 
ein sinnvoller Anwendungsbereich für das von den Mitarbeiter:innen genutzte ERP-System ist.
Des Weiteren wurde deutlich, dass die Dokumente, auf die ich mich im Rahmen der Thesis beschränkt habe, 
viele verschiedene Formen besitzen und teilweise beliebig gewählt werden können.
Die Unterschiede zwischen den einzelnen Dokumenten liegen mehrheitlich in den Details der einzelnen Extraktionsbereiche.
Das Data-Mining dafür erwies sich mit der passenden Extraktionsbibliothek als hilfreich, 
da die Daten für das System und deren Logik greifbarer und leichter zu verarbeiten waren als zum Beispiel über gängige OCR Erkennung.

Eine Anpassung an die Definitionen des Dokumentenmusters muss in der symbolischen künstlichen Intelligenz 
durch eine Kategorisierung in Dokumentarten vorgenommen werden.
Im Zuge dieser Anpassung fiel auf, dass sowohl die Definitionen von Tabellen in elektronisch verarbeiteten Dokumenten, 
als auch die Erkennung dieser Tabellen sich als recht umständlich erweist, da die nötige Standardisierung einer Tabelle fehlte.
Verschiedene Semantiken für die einzelnen vorgestellten Felder benötigen eine weitere Differenzierung der Extraktion.
...
Das System, das ich im Rahmen dieser Thesis entwickelt habe, erwies sich als Prototyp späterer Systeme mit derselben Aufgabe.
Die Struktur für die Teilsysteme wurde durch eine container-getriebene Entwicklung modular gehalten 
und sorgte durch die Architektur für eine gemeinsame Kommunikationsebene für diese Komponenten.
So konnte ein Projekt aufgesetzt werden, welches die neuesten Technologien in den Bereichen, in denen diese vorkommen und agieren, 
nutzte, um die erwünschten Extraktionsziele zu erhalten oder mindestens eine Annäherung zu erreichen. 

Der Webansatz für die Entwicklungsumgebung zeigte bis auf die zusätzliche Build-Zeit 
während des initialen Ladens eine konsistente Verwendung und kein fehlerhaftes Verhalten auf. 
Zudem war die Entwicklung einer Grammatik für den Anwendungsfall wirksam, 
da sich durch die Entwicklung eine Sprache für die Ausdrücke entwickeln ließ, 
mit welcher die Benutzer:innen und die Entwickler:innen schnelle und simple Regeln konfigurieren konnten.
Die Verarbeitung der Dokumente wird durch die daraus generierten Webanfragen ermöglicht, 
sodass der Durchstich der Funktionalitäten stattfindet.

Des Weiteren können die Unterschiede bei Veränderungen der Regeln und Fakten angezeigt werden, 
was die iterative Entwicklung dieser Konfigurationen anbietet.
Für die Umsetzung des Machine-Learning Ansatzes erwiesen sich der Ansatz 
des schwachen maschinellen Lernens als eine vielversprechende Alternative.
So war die Umsetzung einer Pipeline für die PDF Dokumente möglich.
Dafür wurden für jede Dokumentenart Trainings- und Testsätze der Dokumente genutzt, 
um die Supervision und Klassifikation zu nutzen.

Die Datenbank wurde mit Informationen durch die KI befüllt, wodurch sowohl Tabellen für die Erwähnungen und Kandidaten, 
als auch für die einzelnen Felder, die für die Extraktionsfunktionalität definiert wurden, erstellt.
Hierbei habe ich beobachtet, dass über einen längeren Zeitverlauf die Tabellen der Sätze und Paragraphen zuerst genutzt wurden, 
um die multimodale Modellierung der Dokumente abzubilden.
Für den weiteren Verlauf zeigte sich aber auch, dass das genutzte Machine-Learning Framework sehr viele Ressourcen benötigt.
Trotz Virtualisierung traten bei einem längeren Trainingsdurchlauf Abstürze auf, 
welche die Einträge in der Datenbank trotz Wiederherstellung über Mounting beeinträchtigten.


\section{Testing}\label{sec:test}

Für die prototypische und zukünftige Entwicklung des Projekts ist das Testing für die weitere Benutzung relevant. 
Hauptsächlich hierfür verantwortlich soll die Continuous Integration Pipeline sein.
Zu Grunde liegen hier die Images der Teilsysteme, die als Dockerfile beschrieben sind 
und in der Container Registry vom unternehmensinternen Repository gespeichert werden.
Über verschiedene Stages sollen dann die Hauptaspekte der Teilsysteme in das System integriert werden.
Hier werden diese Komponente auf verschieden Eigenschaften überprüft.

Für das Teilsystem der symbolischen KI wurden das Unit Testing 
und das nicht funktionale Testing über die Continuous Integration benutzt.
Für die Entwicklungsumgebung wurde der Ansatz der UI Tests gewählt, um nicht nur die Benutzung 
sondern auch die mögliche Automatisierung für weitere Eingaben zu überprüfen.
Bei dem für das maschinelle Lernen zuständige Teilsystem wurde das Testing über die Testsätze, 
die durch das beschriebene ERP-System bereitgestellt wurden, ermöglicht.
...
aus diesen neue Daten zu generieren. 
...
Diese KI ermöglichte mir während meines Arbeitsprozesses in jedem Schritt eine 
Feinschleifung meiner bisherigen Arbeit der manuellen Extraktion. 
Durch diese KI kann sich jede/r Benutzer/in meines hier entwickelten Systems grobe bis zu detaillierten 
und angepassten Definitionen mit Hilfe einer eigenständigen Umgebung erstellen. Jene Definitionen erlauben dem/der Benutzer:in 
mittels Feedback des Systems eine Annäherung. 
Im späteren Verlauf meiner Arbeit wurde der Ansatz diese Annäherung im Teilsystem des Machine Learnings betrachtet.
Bevor meine symbolische KI als auch das Machine Learning starten konnte, 
habe ich zu Beginn die Funktionalitäten der Bibliotheken genutzt, 
um durch Extraktionsergebnisse der Datenansätze die Erfolgsquote zu überprüfen. 
Danach definierte ich das Format für die Regeln und Fakten des Extraktionsvorgang unter Zuhilfenahme einer eigens entworfenen Grammatik. 

Anschließend stellte ich die Benutzungsoberfläche fertig, sodass ich diese mit der symbolischen KI verbinden konnte. 
Zeitgleich konzeptionierte und entwickelte ich die zu dem Machine Learning gehörende Subsysteme. 
Dies tat ich in einem iterativen Ansatz, sodass eine Pipeline-Struktur erbaut wurde, mit der
das Training des Machine Learnings baldmöglichst beginnen konnte. 
Im Anschluss daran fing ich an, mich in dem gegebenen Rahmen mit dem Software-Testing zu beschäftigen. 
Dadurch konnte die Ausführung aller meiner noch folgenden Prozesse gewährleistet werden. 
Als letzte Vorbereitung passte ich die grafische Benutzungsoberfläche weiter an, dass sich dieses mit den Systemen verbindet, 
wodurch die Ausführbarkeit der Funktionalität überprüft werden konnte.

Durch meine Arbeit habe ich es geschafft, ein System zu schreiben, das die Extraktion von PDF-Dokumenten ermöglicht. 
Dafür habe ich insgesamt zwei verschiedene Ansätze verwendet. 

Während der Entwicklung meines Systems stieß ich allerdings auch auf einige Komplikationen, die es in Zukunft noch zu bewältigen 
und zu verbessern gilt.
So ist unter anderem der Extraktionsvorgang äußerst abhängig von der Semantik des Extraktionsziels des jeweiligen Dokumentationstyps.
In zukünftigen Arbeiten sollte das System deshalb so entwickelt werden, dass weitere Dokumentenarten ihre angepasste Extraktionslogik 
erhalten. Durch diese Weiterentwicklung könnten Zeit, als auch Ressourcen gespart werden.
Darüber hinaus müssen, damit die Sprache für die Regelausdrücke erweitert werden kann, in Zukunft weitere Operatoren 
als auch die Klammerung in der Grammatik definiert werden. 

Andernfalls bleibt die Sprache mit ihren jeweiligen Ausdrücken beschränkt,
sodass mein System auch nur begrenzt nutzbar wäre.
Zuletzt bedarf es für das maschinelle Lernen noch viele weitere Trainingseinheiten für andere Dokumentarten und 
eine weitere Anpassung der Label-Funktionen. Aufgrund des begrenzten Zeitraums, in dem mein System entwickelt worden ist, ist das
Training des Machine Learnings nicht ganz so fortgeschritten, wie ich es zu Beginn meiner Thesis erhofft hatte. Dementsprechend
gilt es, das Machine Learnings in den nächsten Monaten fortzusetzen, sodass weitere Anpassungsleistungen von Seiten des Machine  Learnings 
erbracht werden.

Letztendlich habe ich ein System konzipiert und umgesetzt, das Entwickler:innen die Extraktion von Dokumenten für Mitarbeitende 
in Autohäusern ermöglicht. In der immer mehr digitalisierten Zukunft kann das von mir konzipierte System jedoch nicht nur auf 
Dokumente aus Autohäusern angewendet werden, sondern auch auf Dokumente jenseits der Automobilindustrie. 
