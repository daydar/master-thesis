...
des von der Inference Corporation entwickelten Expertensystems ART für die Regeln und Fakten abgeleitet wurde und damit dieser sehr ähnelt~\cite{haleyaiHaleyARTSyntax2008}. 
Die Umsetzung wird ebenso mit dem Wrapper `clipspy' für die Programmiersprache Python zur Nutzung bereitgestellt.

Eine Alternative zu CLIPS, die zeitlich danach entwickelt wurde, ist das Expertensystem NEXPERT OBJECT, 
...
Folglich können Projekte wie Qt~\cite{qtQtCrossplatformSoftware} und ImGui~\cite{omarDearImGui2022}, 
...
Kandidaten sind die zurzeit konventionellen Frameworks wie TensorFlow~\cite{tensorflowTensorFlow}, pyTorch~\cite{pytorchPyTorch}
und spaCy~\cite{explosionSpaCyIndustrialstrengthNatural}.
...
Die darin gegliederten Komponenten werden `Rule System', `IDE' und `ML' genannt.
...
Für eine Verallgemeinerung der Extraktionsfunktionalitäten wird daher eine gemeinsame Klasse namens `DocumentExtractionType' definiert. 
...
So werden daraus folgend die Dokumentarten, die im Kapitel~\ref{sec:dokart} vorgestellt wurden, durch die Klassen `CareWarranty', `Formel1Invoice' und `KFZ3000Invoice' chronologisch
dargestellt.
...
Die Teilmengen inkludieren jweils auch die leere Menge und besitzen als Elemente die Extraktionsziele.
Für jedes Extraktionsobjekt gibt es Eigenschaften, denen Werte zugewiesen werden.
Ein Fakt ist daher eine Wertezuweisung einer Eigenschaftes eines Extraktionsziel.

Regelbasierte Systeme wie CLIPS nutzen, wie bereits beschrieben, eine angepasste Version der Syntax von der Programmiersprache Lisp.
...
Die Funktion literal\_eval() aus dem Python-Modul `Abstract Syntax Tree' (ast) erlaubt 
...
durch das Auswerten von Ausdrücken die Bildung einer Semantik.
...
Das für dieses Pojekt entwickelte regelbasierte System für die Komponente der symbolischen KI 
...
in der Erweiterten Backus-Naur-Form (EBNF) erlaubt und basierend auf dieser abstrakte Syntaxbäume aus Texteingaben generiert.
...
der Logik für die Evalution dieser Regel bereitgestellt wird.
...
welche wiederum aus der Regel für die Konjuktion oder für die Oder-Verknüpfung aus Disjunktion und Konjunktion ersetzt werden kann.
Die Konjunktionsregel `conjunction' kann entweder durch die `'
Diese Disjunktionsregel beinhaltet wiederum eine eine Konjuktion oder 
So kann zunächst jede Aussage durch die disjunci. 
Eine Aussage wiederum kann als Disjunktion dargestellt werden.
