...
So erlauben unter anderem PDFMiner~\cite{unixuserPDFMiner} oder Apache Tika~\cite{apacheApacheTikaApache} Texte aus 
PDF Dokumenten zu gewinnen. 
...
Es hält eine Kommandozeile, die Möglichkeit, visuelles Debugging zu nutzen (siehe Abbildung ...), 
und Funktionalitäten wie das Entfernen von Duplikaten in Texten, bereit.
Letzteres genannte Feature kann dann zum Einsatz kommen, wenn Dokumente bei ihrer Erstellung oder Verarbeitung 
versehentlich die selben Einträge nochmal im Quellcode des PDFs besitzen. 
...
Bei diesem digitalen Schriftstück handelt es sich zunächst von der Form her um eine Garantierechnung des Care Warranty DMS.
...
Bei den Einträgen in diesem Abschnitt handelt es sich um die Abrechnungsnummer, das Datum der Reparatur, den Fahrzeugmodellnamen, 
den zuletzt abgelesen Kilometerzählerstand, der Arbeitsauftragsnummer, der Fahrzeug-Identifikationsnummer beziehungsweise der
`Vehicle Identification Number' (VIN) und das Datum der Erstellung des Arbeitsauftrags.

Dagegen wird im unteren Bereich eine Tabelle durch die Aufzählung der einzelnen Jobs und Joblines dargestellt.
Die Tabelle ist an der Kopfzeile zu erkennen, welche die Jobs und Joblines in den Arbeitszeitwert beziehungsweise in die Teilenummer, 
in der Bezeichnung, in die Menge des Arbeitszeitwertes, in den Preis und in den Betrag übersichtlich aufteilt.
Der Arbeitszeitwert ist eine Einheit, die für eine genaue Abrechnung der Arbeitszeiten, 
...
in mehreren Zeileneinträgen Überläufe bei den Arbeitszeitwerten bzw. den Teilenummern zu erkennen.
...
Dies ist durch die Codierung der Arbeitszeitwerte bzw. der Teilenummer zu erkennen 
und die Hierarchie ist durch das jeweilige Dealer-Management-System vorgegeben.
So fangen die Codes der Jobs mit `00' an und Reparaturzeiten mit besitzen eine sechsstellige Zeichenkette aus Ziffern.
...
Diese Tabellenform besitzt außerdem selten Trennlinien, um die eine derartige Tabelle als solche definieren zu können.
...
bricht in den prototypartigen Dokumenten aber mindestens einmal die Form durch Überläufe oder Verschiebungen der Texte.
...
Damit das System für den Prozess der Extraktion als solches sich zeitnah umsetzen lässt, 
können Frameworks dem ganzen entgegenkommen.
