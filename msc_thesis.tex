...
Für die vierte Phase findet das schwache überwachte Lernen und die Klassifikation über Labeling-Funktionen statt.
...
So wurde durch die Analyse gezeigt, dass eine weitere Verarbeitung der aus den Autohäusern entstandenen Dokumente 
einen sinnvollen Anwendungsbereich für das von den Mitarbeiter:innen genutzte ERP-System verfügt.
Des Weiteren ist aufgefallen, dass die Dokumente, auf die sich im Rahmen der Thesis eingeschränkt wurden, 
viele verschiedene Formen besitzen beziehungsweise teilweise beliebig gewählt werden können.
Des Weiteren liegen die Unterschiede bei der Mehrheit in den Details der einzelnen Extraktionsbereiche.
Das Data-Mining dafür erwies sich mit den passenden Extraktionsbibliothek als hilfreich, 
da die Daten für das System und deren Logik greifbarer waren.
...
als auch die Erkennung dieser Tabellen sich als umständlich erweist.
...
Der Webansatz für die Entwicklungsumgebung zeigt bis auf die zusätzliche Build-Zeit 
während des initialen Ladens eine konsistente Verwendung und kein fehlerhaftes Verhalten auf. 
Die Entwicklung einer Grammatik war wirksam für den Anwendungsfall, 
da sich dadurch eine Sprache für die Ausdrücke entwickeln ließ, 
mit welcher die Benutzer:innen beziehungsweise die Entwickler:innen schnelle und simple Regeln konfigurieren konnten.
Die Verarbeitung der Dokumente wird durch die daraus generierten Webanfragen ermöglicht, 
womit der Durchstich der Funktionalitäten stattfindet.
...
Hierbei wurde beobachtet, dass über einen längeren Zeitverlauf die Tabellen der Sätze und Paragraphen zuerst genutzt wurden, 
um die multimodale Modellierung der Dokumente abzubilden.
Für den weiteren Verlauf zeigte sich aber auch, dass das genutzte Machine-Learning Framework sehr ressourcenfressend ist.
...
und das nicht funktionale Testing über die Continuous Integration unterstützt.
...
und angepassten Definitionen über ein eigenständige Umgebung erstellen, welche über Feedback des Systems eine Annäherung erlaubt. 
...
Danach definierte ich das Format für die Regeln und Fakten des Extraktionsvorgang mit Hilfe einer eigens entworfenen Grammatik. 

Anschließend stellte ich die Benutzungsoberfläche fertig, sodass ich diese mit der symbolischen KI verbinden konnte. 
Nebenbei konzeptionierte und entwickelte ich die zu dem Machine Learning gehörende Subsysteme. 
Dies tat ich in einem iterativen Ansatz, sodass eine Pipeline-Struktur erbaut wurde womit
 das Training des Machine Learnings baldmöglichst beginnen konnte. 
Im Anschluss daran fing ich an mich in dem gegebenen Rahmen mit dem Software-Testing an zu beschäftigen. 
...
Meine Ergebnisse dieser Arbeit zeigten einige Komplikationen auf.
So ist zu einem der Extraktionsvorgang sehr abhängig zur Semantik des Extraktionsziels.
Hier kann die entwickelte Struktur dafür sorgen, dass weitere Dokumentenarten ihre angepasste Extraktionslogik bekommen.
