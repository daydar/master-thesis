...
Genau jenes Bedürfnis nach Digitalisierung betrifft auch die vielen Arbeitnehmer:innen / Händler:innen in Autohäusern und deren Kfz-Werkstätten, 
...
Jene Arbeitnehmenden in ausgewählten Autohäuser und deren Kfz-Werkstätten sind Kund:innen bei ilexius GmbH. 
...
vereinfacht werden. In Kooperation von ilexius GmbH werde ich daher diese Problematik lösen und in die Tat umsetzen.
...
bereits elektronisch aufgearbeitet und den Arbeitnehmenden in Autohäusern und deren Kfz-Werkstätten zur Verfügung gestellt worden. 
...
was die Daten an sich darstellen sollen \cite{PDFScraperScrape}. 
...
Hier werden dann die Komponenten zusammengeführt um ein gemeinsames System zu ermöglichen, 
welches dann gegebenenfalls weitere Testschritte bedarf. 
Sobald diese eine erfolgreiche Form annimmt kommt der Teil der Continuous Integration hinzu, 
womit das System einen Zustand der Betrieblichkeit annehmen kann. 
Über Testing sollen so mögliche Fehlerquellen entdeckt werden bevor diese auftreten können.

Unter die Zielsetzung für die beiden Ansätze fallen die Kundendaten mit den Eigenschaften wie 
...
Für das Mengengerüst sollen mehr als 100.000 Dokumente pro Jahr später bearbeitet werden. 
...
Hierbei sollen die möglichen Technologien abgewogen und evaluiert werden. 
Zunächst einmal wird der Kontext ausführlicher eklärt.

\section{Kontext}\label{sec:kon}

Im Automobilbereich wird der Verkauf von Kraftfahrzeugen in zwei Segmente aufgeteilt: dem Sales-Bereich, indem es sich um den Verkauf 
von Neu- und Gebrauchtwagen dreht und dem After-Sales-Bereich, indem eine Bindung zum Kunden über den Verkauf von 
Verschleiß- und Ersatzteilen geschaffen wird. \cite{theuerer_was_nodate}
Diese Aufteilung der Segmente wird auch von den Autohäusern angewendet und für diese werden jeweils gängige Abläufe als Prozesse benutzt.

Für den After-Sales-Bereich bietet ilexius GmbH zwei Arten von ERP-Systeme an. 
Die erste Art ist das von der Automobilmarke Jaguar Land Rover finanzierte 'vTab', welches eine Plattform für elektronische Vehikelchecks 
und ein Backend für Jaguar Land Rover Mitarbeitern, womit die Autohäuser bewertet werden können, anbietet 
und die zweite Art namens 'ATT' ist eine Plattform, womit Autohäuser ihre Arbeitsprozesse in Schrift, Sprache und Bild dokumentieren können.
Die Dokumentation folgt durch mobile Endgeräte und werden an das ERP-System weitergeleitet um diese zu archivieren.

Die ERP-Systeme, die bereits im Kapitel \ref{sec:ziel} beschrieben wurden, unterstützen damit die Kunden:innen von ilexius GmbH 
in den Autohäusern und deren Werkstätten mit der Planung, der Steuerung und der Verwaltung von verschiedenen Aufgaben in diesem After-Sales-Bereich.

Eine Art der Aufgaben sind die Arbeitsaufträge, bei der es sich in den meisten Fällen um die Behandlung eines Autos samt Kundendaten handelt. 
Diese wiederum unterteilen sich in einzelne Jobs die den ganzen Auftrag in einzelne Teilschritte zur Vervollständigung des Arbeitsauftrags führen.
...
Das ERP bietet des Weiteren eine Schnittstelle für diese Dokumente wie z.B die Rechnungen an, die nur über Scanner die Dokumente übertragen. 
Dort werden die Dokumente verarbeitet und mit Hilfe eines OCR Scanners digitalisiert. 

Aktuell ist aber die Digitalisierung nicht in der Lage einen Kontext beziehungsweise (bzw.) einen Semantik für diese Dokumente zu erstellen.
Somit bietet sich hier an der Stelle die Möglichkeit einen neuen Ansatz auszuprobieren um die Informationen aus diesen Dokumenten als Daten zu erhalten.
...
oder Software as a Service (SaaS) Plattformen zu beschaffen \cite{stichdata_what_nodate}. 
...
So nutzen Produkte wie Textractor \cite{amazon_intelligente_nodate} von AWS oder Adobes Extractor API 
\cite{developers_extract_nodate} Künstliche Intelligenz, Machine Learning und Optical Character Recognition (OCR) 
um die Extraktion der Daten zu ermöglichen. 
Unternehmen wie ABBYY \cite{abbyy_pdf_nodate} habe sich bereits auf diesem Gebiet spezialisiert 
...
So erlauben unter anderem PDFMiner \cite{unixuser_pdfminer_nodate} oder Apache Tika \cite{apache_apache_nodate} Texte aus PDF Dokumenten zu gewinnen. 
Funktionalitäten für das Extrahieren von Daten aus Tabellen bieten Projekte wie 
Tabula \cite{tabulapdf_tabulapdftabula_nodate} oder Camelot \cite{camelot_camelot_2022}
an womit ein weiterer Aspekt durch die Nutzung abgedeckt wäre. 

Bei Beiden zeigen sich niedrige Fehlerquoten auf \cite{atlanhq_comparison_nodate} und die Nutzung von einer Fuzzy Logik. 
Camelot ermöglicht dazu noch ein Web Interface womit die simple Extraktion über einen Browser lokal stattfinden kann.
Andere Bibliotheken wie Textricator \cite{measures_measuresforjusticetextricator_nodate} sind umfangreicher gestaltet und ergeben geeignete Schnittstellen für die Anfragen 
spezifischer Inhalte. 
Doch auch wenn dies mit textbasierten Dokumenten funktioniert und der Export flexibel ist, 
können weder nicht-textbasierte noch komplexere PDF Dokumente verarbeitet werden. 
